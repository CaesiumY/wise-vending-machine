...
const PRESET_STYLES: Record<PresetName, { color: string; icon: string }> = {
  normal: { color: "", icon: "" },
  change_shortage: { color: "", icon: "" },
  stock_shortage: { color: "", icon: "" },
  system_error: { color: "", icon: "" },
  worst_case: { color: "", icon: "" },
};
        ...
        {SCENARIO_PRESETS.map((preset) => {
          ...
          return (
            <Button
              ...
              className={cn(
                "h-auto p-3 flex items-start gap-3 text-left",
                style.color,
                activePreset === preset.name && "ring-2"
              )}
            >
