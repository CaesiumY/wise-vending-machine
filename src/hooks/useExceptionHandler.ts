...
interface ExceptionResult {
  type: ErrorType
  message: string
  recovery: 'return_money' | 'restore_payment' | 'maintenance_mode' | 'safe_mode' | 'retry_insertion' | 'select_other' | 'use_cash' | 'try_other_payment' | 'request_exact_amount' | 'return_excess' | 'auto_return' | 'limited_service' | 'service_unavailable' | 'default'
  shouldBlock: boolean
}
...
export function useExceptionHandler() {
  ...
  // 예외 상황 체크 및 처리
  const checkExceptions = useCallback((operation: string, context?: ExceptionContext): ExceptionResult[] => {
    ...
    if (adminState.changeShortageMode && operation === 'calculate_change') {
      exceptions.push({
        ...
        message: '거스름돈이 부족합니다. 정확한 금액을 투입해주세요.',
    ...
    // 3-4. 지폐/동전 걸림
    if ((adminState.billJamMode || adminState.coinJamMode) && operation === 'insert_cash') {
  ...
  // 예외 처리 실행
  const handleException = useCallback((exception: ExceptionResult) => {
    ...
    switch (exception.recovery) {
      ...
      case 'restore_payment':
        ...
        vendingStore.showDialog('info', '결제 복구', '결제를 취소하고 금액을 복구했습니다.')
        break
      ...
      case 'use_cash':
        ...
        setStatus('idle')
        vendingStore.showDialog('info', '결제 방법 안내', '현금 결제를 이용해주세요.')
        break
      ...
      case 'request_exact_amount':
        // 정확한 금액 투입 안내
        vendingStore.showDialog('info', '정확한 금액', '정확한 금액을 투입해주세요.')
        break
      ...
      case 'limited_service':
        // 서비스 제한 안내
        vendingStore.showDialog('error', '서비스 제한', '일부 기능이 제한될 수 있습니다.')
        break
  ...
  // 특정 작업 전에 예외 체크
  const checkAndHandleExceptions = useCallback((operation: string, context?: ExceptionContext): boolean => {
  ...
  // 예외 상황 강제 발생 (테스트용)
  const triggerException = useCallback((errorType: ErrorType) => {
