...
export function useCashPayment() {
  ...
  const insertCash = async (amount: CashDenomination): Promise<boolean> => {
    try {
      // 위조화폐 감지 모드 (30% 확률)
      if (fakeMoneyDetection && Math.random() < 0.3) {
        setError('fake_money_detected', '위조화폐가 감지되었습니다. 화폐를 반환합니다.');
        showDialog('error', '위조화폐 감지!', '화폐를 다시 삽입해주세요.');
        return false;
      }
      ...
      if ((amount >= 1000 && billJamMode) || (amount < 1000 && coinJamMode)) {
        if (Math.random() < 0.4) {
          ...
          setError(jamType, `${amount >= 1000 ? '지폐' : '동전'}가 걸렸습니다. 다시 시도해주세요.`);
          showDialog('error', `${amount >= 1000 ? '지폐' : '동전'} 걸림!`, '다시 투입해주세요.');
          return false;
        }
      }
      ...
      if (result.success) {
        ...
        showDialog('success', '투입 완료', `${amount}원이 투입되었습니다.\n현재 잔액: ${newBalance}원`);
        return true;
      }
  ...
  const getAvailableProducts = () => {
    return Object.values(products).map(product => ({
      ...
      reason: currentBalance < product.price ? '잔액 부족' : 
              product.stock === 0 ? '품절' : null
    }));
  };
  ...
  const returnAllCash = async (): Promise<boolean> => {
    try {
      ...
      if (changeShortageMode && returnAmount > 0) {
        const changeResult = calculateOptimalChange(returnAmount, cashInventory);
        
        if (!changeResult.possible) {
          setError('change_shortage', '거스름돈이 부족합니다. 정확한 금액을 투입해주세요.');
          showDialog('error', `거스름돈 부족!`, `${changeResult.shortage ? '일부' : '전액'} 반환할 수 없습니다.`);
          return false;
        }
      }
...
export function useAdmin() {
  ...
  return {
    ...
    getSystemSummary: () => {
      const activeExceptions = Object.entries(adminStore)
        .filter(([key, value]) => 
          ...
          value && 
          key.endsWith('Mode') || key.includes('Fault') || key.includes('Detection')
        )
        .map(([key]) => key);
    ...
    runTestScenario: (scenarioName: string) => {
      adminStore.loadPreset(scenarioName as 'normal' | 'change_shortage' | 'stock_shortage' | 'system_error' | 'worst_case');
